...
/******** Begin Time Routines *********/
  ...
  doCurrentTime();                        // do routine to display current time
...
/* calculate amps, volts, and watts for this loop */
  float finalA = getChargeAmps();                   //= averageA/100;    // divide by 100, the number of samples taken
  float finalV = getChargeVolts();                  //= averageV/100;    // divide by 100, the number of samples taken
  ...
  delay(500);                                // wait 1/2 second then do loop again
}
/************************************
*             END LOOP              *
*                                   *
...
*****************************************************************/
float getChargeAmps() {
  float averageA = 0.0;                                  // zero out average before the loop
  float loopCurrent = 0.0;                               // zero out before the loop
  int count = 0;                                         // count counts
  for(int x = 0; x < 100; x++){                          // take 100 samples for average current reading
    ...
    count = x;                                           // add to counts
    delay(10);                                           // 10 mSec between each reading
  }
  averageA = averageA/count;                             // divide by count to get average
  return averageA;                                       // return average
}
...
* Added offset of .2v to match with voltmeter reading         *
* Multiply voltage by 16 for correct voltage                  *
***************************************************************/
float getChargeVolts() {
  float averageV = 0.0;                                   // zero out average before the loop
  float loopVoltage = 0.0;                                // zero out before loop
  int count = 0;                                          // count counts
  for(int y = 0; y < 100; y++){                           // take 100 samples for average voltage reading
    voltageValue = analogRead(0);                         // read voltage sensor
    loopVoltage = ((voltageValue * .00488281) * 16) + 0.2;        // convert to voltage
    averageV = averageV + loopVoltage;                    // calculate average voltage in this loop
    count = y;                                            // add to counts
    delay(10);                                            // 10 mSec delay between each reading
  }
  averageV = averageV/count;                              // divide by counts to get average
  return averageV;                                        // return average
}
...
void doCurrentTime() {
  ...
  if (nowHour == 0) {                                     // if it's the midnight hour
    int midnightHour = 12;                                // use the number 12 instead of 0 for 12:00 to 12:59 am
    ...
    if(eveningHour < 10) {                                // if it's between 1:00 and 9:59 pm
      clockHour = ' ' + String(eveningHour);              // add leading space
    } else {
      clockHour = String(eveningHour);                    
    }
  } else {
    clockHour = String(nowHour);                          // if we're here it can only be 10 or 11 am
  }
